...
class LE_Processor(Processor):
    ...
    def load_optimizer(self):
        #ignored_params = list(map(id, self.model.encoder.fc.parameters()))
        #base_params = filter(lambda p: id(p) not in ignored_params, self.model.encoder.parameters())
        ...
        if self.arg.optimizer == 'SGD':
            ...
            #self.optimizer = optim.SGD(
                #[{'params': base_params, 'lr': 0.1},{'params': self.model.encoder.fc.parameters(), 'lr': 3.0}],
                #lr=0.1,
                #momentum=0.9,
                #nesterov=self.arg.nesterov,
                #weight_decay=self.arg.weight_decay)
